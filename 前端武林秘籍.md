...
团队有三个前端，主要负责新项目的搭建，技术栈的选型，主要页面的开发，单元测试的编写，公共组件的开发，比如侧边栏，页面公共布局，搜索栏，还开发了一个含有产品价格的日历组件，当用户切换月份的时候，可以动态的查询相对应日期的产品价格，并插入对应的日期下，这样用户在订房的时候就一目了然，还有开发一些公共插件，比如验证插件，里面包含了一些基本的验证规则，比如 required，range，fax，mobile等，也支持用户传入一个回调函数自定义验证规则，还支持异步验证等。
然后对整个前端项目性能的把控，比如减小入口文件大小，组件异步加载，非必要组件懒加载，必要组件预加载，服务端设置gzip，etag，
全站图标资源统一管理，比如雪碧图的应用，图标字体 iconfont 的应用，全站静态资源的 cdn 加速，
然后还会在项目中加入百度统计，统计用户对每个按钮和组件的操作，这样就能知道哪些组件是用户很少用到的功能，后期就可以进行针对性的优化。