...
四、Vue的双向数据绑定实现原理
	...
	Object.defineProperty缺陷的：
		...
		2.对于数组不能监听数据的变化
		而proxy原生支持监听数组的变化，并且可以直接对整个对象进行拦截，所以Vue在下个版本中用proxy替换object.defineProperty
...
十、谈谈Vue和React组件化的思想
	...
	2.随着业务逻辑变得越来越多，我们就想要我们的组件可以处理很多事，这就是我们常说的组件化，这个组件就不是UI组件了，而是包具体业务的业务组件
...
十、TCP三次握手	(客户端和服务器端都需要确认各自可收发）
	...
	客户端发c起请求连接服务器端s确认，服务器端也发起连接确认客户端确认。
