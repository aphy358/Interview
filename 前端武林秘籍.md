...
自己的职业规划：
想想最后问面试官的问题：你们公司用什么技术栈？你们的前端有独立的团队吗，有多少人？
...
二、什么是闭包和原型链
	...
	执行上下文：一个看不见的对象，存在若干个属性和变量，它被调用的时候创建的。函数被调用查看的this指向的object，object就是上下文（只有被调用的时候创建）
	   ...
	   原型对象用Object.prototype.__proto__ = null表示原型链顶端。如此形成了js的原型链继承。同时所有的js对象都有Object的基本方法
...
一、介绍下MVVM(数据的双向绑定）
	...
	vue 相比于传统开发方式，主要是避免了频繁操作 DOM 带来的繁琐，通过指令绑定数据和事件，当数据改变时，其他关联该数据的组件做出响应并更新。
	其核心思想是通过数据劫持 + 发布/订阅者模式实现双向数据绑定。

	目录结构包含 api + pages + components + router + store + main
...
十一、webpack
	...
	常用的 plugins：commonchunkPlugin、ExtractTextPlugin、OptimizeCSSPlugin、HtmlWebpackPlugin
...
http://www.bslxx.com/m/view.php?aid=2145




介绍自己：
本人从事前端工作已经四年多，现在团队有三个前端，项目涉及PC端酒店预订系统、还有移动端的微信公众号，我的工作主要是负责新项目的搭建，技术栈的选型,公共组件的开发，比如工具函数，页面布局，侧边栏，搜索栏等，然后是对整个项目的性能把控，比如减小入口文件大小，组件异步加载，非必要组件懒加载，必要组件预加载，服务端设置gzip，etag等，再就是图标资源统一管理，比如雪碧图的应用，图标字体 iconfont 的应用等，还有前端静态资源的缓存以及cdn加速。有必要的话还可以在项目中加入百度统计，统计用户对每个组件的操作，这样就能知道哪些组件是用户很少用到的，后期就可以进行针对性的优化。
在最近的一年多时间里，我除了负责自己的开发任务以外，还负责对团队其他成员的任务分配，指导和协调团队开发。
在工作以外，我也非常乐于学习新的技术，平时经常在 github 上倒腾自己的小项目，或者开发一些自己用的插件，比如带产品价格的日历插件，就是在相应的每一天日期下面插入产品价格，这样用户就更加一目了然。还有表单验证插件，里面包含一些常用的验证规则，像必填、最大长度、最小长度、邮箱、电话号码等，还可以传入一个回调函数自定义验证规则，还支持异步验证。然后还写了其他一些插件，像实现元素的拖拽啊等等。


你是怎么搭建vue项目的：
首先可以借助vue的脚手架把最基本的框架搭好，这个过程中可以设置好单元测试框架，样式预处理的loader，eslint等。然后可以安装自己在项目中会用到的插件，像 element UI、lodash、verlocity、karma、mocha、chai 等等，接下来设计项目目录结构，大致有入口文件 main.js、路由router、store、components、pages、api、utils，前期开发一些基础公共组件，然后再接着开发各个页面的内容。