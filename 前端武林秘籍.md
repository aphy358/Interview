...
你是怎么搭建vue项目的：
首先可以借助vue的脚手架把最基本的框架搭好，这个过程中可以设置好单元测试框架，样式预处理的loader，eslint等。然后可以安装自己在项目中会用到的插件，像 element UI、lodash、verlocity、karma、mocha、chai 等等，接下来涉及项目目录结构，大致有入口文件 main.js、路由router、store、components、pages、api、utils，前期开发一些基础公共组件，然后再接着开发各个页面的内容。